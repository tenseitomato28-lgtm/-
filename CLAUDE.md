...
2. **X自動投稿**: dlvr.it が `feed.xml` を監視し、新記事をXに投稿する構成。**ただし2026/7/28時点で停止中**
   - **停止の原因（7/28判明）**: dlvr.it のルートが旧GitHub Pages の feed URL（`tenseitomato28-lgtm.github.io/-/feed.xml`）を見たままで、7/22のCloudflare Pages移行でその URL が404になり配信が止まった。**復旧にはdlvr.it管理画面でフィードURLを `https://murinaku-blog.pages.dev/feed.xml` に差し替える必要がある**（ユーザーの手作業。Claudeからdlvr.itには到達できない）
   - **7/22〜7/28に公開した6本はXに自動投稿されていない**。必要なら手動投稿する
   - **教訓**: 公開ドメインを変えたら dlvr.it 側のフィードURLも必ず更新する。dlvr.it は管理画面がClaudeから見えないため、止まっても気づけない。**「接続済み」と書かれていても、実機で確認が取れるまで動作を前提にしない**
